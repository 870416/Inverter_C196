...
8255口分配:
  ...
  PB口:输出口,显示驱动,对照表(B口数据/显示数字)
    0/3FH,1/30H,2/5BH,3/4FH,4/66H,5/6DH,6/7DH,7/07H,8/7FH,9/6FH.
    A./F7H,b./FCH,C./B9H,d./DEH,E./F9H,F./F1H,H./F6H,L./B8H,O./DCH,P./F3H,U./BEH.
  PC口:
    PC7-PC1:未用;
...
P2口:
  ...
  P2.7:DS1820口线;
*******************************************************************/
#ifndef DA502_DATA_HH
#define DA502_DATA_HH
...
extern UI       s_x2,s_time0;
extern BYTE     s_control,s_ok,s_send; 
/* 7/首次计算，6/数据有效，5/当前发送区，4/未用，*/
/* 3/升压允许，2/升频    ，1/逆变允许  ，0/逆变过程，*/
/* 为提高编译效率，将位6，5单列为s_ok，s_send */
typedef struct
{
	BYTE   head1;/* 0x55 */  
	BYTE   u1;   /* 前级电压 */
    ...
    BYTE   tb ;  /* 机箱温度 */
    BYTE   fend; /* 目标频率 */
    BYTE   send; /* 目标工况,32/端口,10/工况:00-停机,01-通风,10-制冷,11-制热 */
    BYTE   fnow; /* 实际频率,fnowc/10, */
    BYTE   snow; /* 实际工况：7/电热，6/压缩，5/冷凝，4/通风，低位同上 */
    BYTE   ec1 ; /* 7/命令无效，6/C通讯错 ，5/B通讯错 ，4/A通讯错  */
                 /* 3/电热过流，2/压缩过流，1/冷凝过流，0/通风过流 */
    BYTE   ec2 ; /* 内部故障代码 */
    BYTE   end1; /* 0xfd */
    
    BYTE   sendm;/* 机组允许工况，同上，高4位不用 */

    UI     fendc; /* fend×10,目标频率(控制用) */
    UI     fnowc; /* 实际频率(控制用) */
    UI     foldc; /* 上次频率(控制用) */
...
typedef struct
{
	...
	BYTE   wenda; /* A端目标工况， 7654/频率偏移，F=50-X， */ 
	BYTE   wendb; /* B端目标工况， 7654/频率偏移，F=50-X， */ 
	BYTE   wendc; /* C端目标工况， 7654/频率偏移，F=50-X， */ 
